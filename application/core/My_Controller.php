...
class RestController extends CI_Controller
{
    ...
    /**
     * Prepares for apikey authentication.
     *
     * @return void
     */
    protected function _prepare_apikey_auth()
    {
        // If whitelist is enabled it has the first chance to kick them out
        if ($this->config->item('rest_ip_whitelist_enabled')) {
            $this->_check_whitelist_auth();
        ...
        if (!in_array($api_key, $this->config->item('rest_valid_api_key'))) {
            $this->response([
                $this->config->item('rest_status_field_name')  => false,
                $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unauthorized'),
            ], self::HTTP_UNAUTHORIZED);
    ...
    protected function _log_access_time()
    {
        ...
        $_end_rtime = $this->_end_rtime ?? 0;
        $_start_rtime = $this->_start_rtime ?? 0;
        $payload['rtime'] = $_end_rtime - $_start_rtime;
